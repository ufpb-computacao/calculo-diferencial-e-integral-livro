...
.. latexmath:[$\dfrac{x-9}{(x-13)\sqrt[4\]{x^2-4}\sqrt{x-1}\sqrt[6\]{x+3}}\leq 0$]

Solução:: Desde que  latexmath:[$n_1=4$, $n_2=2$ e $n_3=6$], ou seja todos pares, podemos aplicar o item i(b) do *Caso VI*, para latexmath:[$P(x)=x-9$, $Q_1(x)=\sqrt[4\]{x^2-4}$, $Q_2(x)=\sqrt{x-1}$, $Q_3(x)=\sqrt[6\]{x+3}$ e $R(x)=x-4$]. Assim, temos que:
+
[latexmath]
++++
\[
\dfrac{x+5}{(x-4)\sqrt[4\]{x^2-4}\sqrt{x-1}\sqrt[6\]{x+3}}\geq 0\quad \Leftrightarrow\quad  81-x^2> 0\quad\mbox{e}\quad\dfrac{x+5}{x-4}\geq 0\]
Por outro lado, pelo item(iii) de {\bf Inequações Irracionais}, temos que
\[
\dfrac{x+5}{x-4}\geq 0\quad \Leftrightarrow \quad (x+5)(x-4) \geq 0\quad \mbox{e}\quad x-4\neq 0
\]
