...
Subtração::  Dados latexmath:[$a$] e  latexmath:[$b\in \mathbb{R}$], a  *subtração*, ou *diferença*, de latexmath:[$a$] e latexmath:[$b$] é definida como latexmath:[$a-b=a+(-b)$].
...
|======================
| latexmath:[$x<y<z\quad $]é equivalente a latexmath:[$\quad x<y$] e latexmath:[$y<z$];
|
| latexmath:[$x<y\leq z\quad$]é equivalente a  latexmath:[$\quad x<y$] e latexmath:[$y\leq z$]; 
|
| latexmath:[$x\leq y< z\quad$]é equivalente a  latexmath:[$\quad x\leq y$] e latexmath:[$y< z$]; 
|
| latexmath:[$x\leq y\leq z\quad$]é equivalente a  latexmath:[$\quad x\leq y$] e latexmath:[$y\leq z$]. 
|======================
...
.Fórmula de Bhaskara
====
Esta fórmula nos ajudará a encontrar as raízes de uma equação de segundo grau. Assim, para a equação de segundo grau:
[latexmath]
++++++++++++++++++++++++++++++++++++++++++++
\[
 ax^2+bx+c=0.
...
temos que
\[
x=\dfrac{-b\pm\sqrt{\Delta}}{2a}, \quad \mbox{com}\quad \Delta=b^2-4a c.
\]
++++++++++++++++++++++++++++++++++++++++++++
onde latexmath:[$\Delta$] é conhecido como o *discriminante*. Assim:
...
+
Portanto, do item b do *Importante* posterior ao *Teorema 1.3*, para latexmath:[$b=-1$, podemos concluir que $x^2+1=0$] não tem raízes em latexmath:[$\mathbb{R}$] e 
+
[latexmath]
++++
\[{\rm C.\,\,S.}=\emptyset.\]
...
+
--
Método 1 (Usando a fórmula de Bhaskara ou o Discriminante latexmath:[$\Delta$]):::
...
... latexmath:[$ax+b>0 \quad \Leftrightarrow\quad  x>-\frac{b}{a}\quad $ se, e somente se, $\quad {\rm C.\,\,S.}=\left(-\frac{b}{a}, + \infty\right)$];

... latexmath:[$ax+b<0 \quad \Leftrightarrow \quad x<-\frac{b}{a} \quad $ se, e somente se, $\quad  {\rm C.\,\,S.}=\left(-\infty, -\frac{b}{a}\right)$];

... latexmath:[$ax+b\geq 0 \quad \Leftrightarrow\quad  x\geq-\frac{b}{a}\quad $ se, e somente se, $\quad {\rm C.\,\,S.}=\left[-\frac{b}{a}, + \infty\right)$];

... latexmath:[$ax+b\leq 0 \quad \Leftrightarrow \quad x\leq-\frac{b}{a}\quad $ se, e somente se, $ \quad {\rm C.\,\,S.}=\left(-\infty, -\frac{b}{a}\right\]$].
...
Solução::
latexmath:[$5x+5\geq 1-3x$] latexmath:[$\,\,\,\Leftrightarrow\,\,\,$] latexmath:[$5x+3x\geq 1-5$] latexmath:[$\,\,\,\Leftrightarrow\,\,\,$]  latexmath:[$8x\geq -4$]  latexmath:[$\,\,\,\Leftrightarrow\,\,\,$]  latexmath:[$x\geq -\dfrac{4}{8}$] latexmath:[$\,\,\,\Leftrightarrow\,\,\,$]  latexmath:[$x\geq-\dfrac{1}{2}$]. 
+
Portanto,
+
[latexmath]
...
Solução::  latexmath:[$3x-4<2+x$] latexmath:[$\,\,\,\Leftrightarrow\,\,\,$] latexmath:[$3x-x<2+4$] latexmath:[$\,\,\,\Leftrightarrow\,\,\,$] latexmath:[$2x<6$] latexmath:[$\,\,\,\Leftrightarrow\,\,\,$]latexmath:[$x<\frac{6}{2}$] latexmath:[$\,\,\,\Leftrightarrow\,\,\,$]  latexmath:[$x<3$]. 
+
Portanto,
+
[latexmath]
...
\]
++++++++++++++++++++++++++++++++++++++++++++
Suponhamos, sem perda de generalidade, que latexmath:[$a>0$]. Assim, usando a *fórmula de Bhaskara*, temos os seguintes casos:

Caso I:: Se latexmath:[$\Delta=0$], então latexmath:[$ax^2+bx+c=0$] tem uma única raiz, isto é, latexmath:[$r=r_1= r_2$]. Portanto:
+
--
... latexmath:[$ax^2+bx+c>0$] se, e somente se, latexmath:[$ {\rm C.\,\,S.}= \mathbb{R}\setminus\{r\}$];
...
+
--
... latexmath:[$ax^2+bx+c>0$] se, e somente se, latexmath:[$ {\rm C.\,\,S.}= (-\infty, r_1)\cup (r_2,+\infty)$];
...
+
--
... latexmath:[$ax^2+bx+c>0$] se, e somente se,  latexmath:[${\rm C.\,\,S.}= \mathbb{R}$];
... latexmath:[$ax^2+bx+c<0$] se, e somente se, latexmath:[${\rm C.\,\,S.}=\emptyset$];
... latexmath:[$ax^2+bx+c\geq 0$] se, e somente se, latexmath:[${\rm C.\,\,S.}=  \mathbb{R}$];
...
++++
\[
x^2-2<3x+2 \quad \Leftrightarrow \quad x^2-3x-4<0\quad \Leftrightarrow \quad(x-4)(x+1)<0.
\]
+++++
+
...
++++
\[
\begin{array}{cclllccccccc}
(x-4)(x+1)<0& \Leftrightarrow &(x-4<0 \quad \mbox{e}\quad x+1>0)\quad \mbox{ou} \quad (x-4>0\quad \mbox{e}\quad x+1<0)\\\\
& \Leftrightarrow &(x<4 \quad \mbox{e}\quad x>-1)\quad \mbox{ou} \quad (x>4\quad \mbox{e}\quad x<-1)\\\\ 
 &\Leftrightarrow &-1<x<4\quad \mbox{ou} \quad \emptyset \quad \Leftrightarrow \quad x \in (-1,4)\cup\emptyset 
\\\\
...
++++
\[
\begin{array}{ccllllllllllllccccccc}
x^2-2<3x+2& \Leftrightarrow &x^2-3x<4 \quad \Leftrightarrow \quad x^2-3x+\dfrac{9}{4}<4+\dfrac{9}{4}\\\\
& \Leftrightarrow &\left(x-\dfrac{3}{2}\right)^2<\dfrac{25}{4}\quad \Leftrightarrow \quad\left(x-\dfrac{3}{2}\right)^2-\left(\dfrac{5}{2}\right)^2<0\\\\
& \Leftrightarrow &\left(x-\dfrac{3}{2}-\dfrac{5}{2}\right)\left(x-\dfrac{3}{2}+\dfrac{5}{2}\right)<0\quad
\Leftrightarrow \quad
\left(x-\dfrac{8}{2}\right)\left(x+\dfrac{2}{2}\right)<0\\\\
& \Leftrightarrow &\left(x-4\right)\left(x+1\right)<0.
\end{array}
\]
++++ 
+
Assim, trabalhando de forma analoga ao Método 1 acima, 
 temos que
+
[latexmath]
++++
\[
\left(x-4\right)\left(x+1\right)<0\quad \Leftrightarrow\quad x \in (-1,4).
\]
++++ 
...
++++
\[
x^2-2<3x+2\quad\Leftrightarrow\quad x^2-3x-4<0\quad\Leftrightarrow\quad (x+1)(x-4)<0.
\]
++++
+ 
Assim, os valores de latexmath:[$x$] para os que latexmath:[$(x+1)(x-4)=0$] são latexmath:[$x=-1$] e latexmath:[$x=4$] (raízes de cada fator). Logo,
+
.Quadro de sinais
...
--
+
... latexmath:[$P(x)\geq 0$]  se, e somente se, latexmath:[$x$] pertence à união dos intervalos fechados com sinal latexmath:[$+$], isto é:
+
--
.. Se  latexmath:[$ n$] é par, então  latexmath:[${\rm C.\,\,S.}= (-\infty, r_1\]\cup \dots\cup [r_n,+\infty)$];
.. Se  latexmath:[$ n$] é ímpar, então  latexmath:[${\rm C.\,\,S.}= [ r_1,r_2\]\cup \dots\cup [r_n,+\infty)$];
...
... latexmath:[$\dfrac{P(x)}{Q(x)}>0\quad \Leftrightarrow \quad P(x)Q(x)>0$];
... latexmath:[$\dfrac{P(x)}{Q(x)}<0\quad \Leftrightarrow \quad P(x)Q(x)<0$];
... latexmath:[$\dfrac{P(x)}{Q(x)}\geq 0\quad \Leftrightarrow \quad P(x)Q(x)\geq 0\quad\mbox{e}\quad Q(x)\neq 0$];
... latexmath:[$\dfrac{P(x)}{Q(x)}\leq 0\quad \Leftrightarrow \quad P(x)Q(x)\leq 0\quad\mbox{e}\quad Q(x)\neq 0$].
...
\\\\
&
\Leftrightarrow &\left(\dfrac{x}{x-1}+ \dfrac{x-1}{x}- \dfrac{2x}{x+1}\right)(x+2)\leq 0
\end{array}\]
No entanto
\[
\dfrac{x}{x-1}+ \dfrac{x-1}{x}- \dfrac{2x}{x+1}=\dfrac{x^2(x+1)+(x-1)(x-1)(x+1)-2x^2(x-1)}{(x-1)x(x+1)}=\dfrac{2x^2-x+1}{(x-1)x(x+1)}
\]
Assim, 
\[\begin{array}{ccll} 
\dfrac{x(x+2)}{x-1}+ \dfrac{(x-1)(x+2)}{x}\leq \dfrac{2x(x+2)}{x+1}&
...
&
 {(2x^2-x+1)(x+2)}{(x-1)x(x+1)}\leq 0\quad  \mbox{e}\quad  (x-1)x(x+1)\neq 0\\\\
\Leftrightarrow &{(x+2)}{(x-1)x(x+1)}\leq 0\quad \mbox{e}\quad  (x-1)x(x+1)\neq 0
\end{array}\]
++++
+
Fazendo latexmath:[$P(x)={(x+2)}{(x-1)x(x+1)}$, temos que as raízes de $P(x)=0$  são $r_1=-2$], latexmath:[$r_2=-1$], latexmath:[$r_3=0$] e latexmath:[$r_4=1$. Pela {\bf nota} da {\bf subseção 1.4.1.4}, $(x-1)x(x+1)\neq 0$ implica que os intervalos que tenham  $r_2$, $r_3$ e $r_4$] devem ser abertos nestes extremos. Desde que a inequação é da forma latexmath:[$P(x)\leq 0$],  podemos aplicar o item iii(a) do *Caso I*, pois  consideraremos todas as raízes. Ou seja, latexmath:[$x$] pertence à união dos intervalos com sinal latexmath:[$(-)$], veja figura a seguir:
+
...
... latexmath:[$a^{f(x)}> a^{g(x)}\quad \Leftrightarrow\quad {f(x)}>{g(x)}$];
... latexmath:[$a^{f(x)}< a^{g(x)}\quad \Leftrightarrow\quad {f(x)}<{g(x)}$];
... latexmath:[$a^{f(x)}\geq  a^{g(x)}\quad \Leftrightarrow\quad {f(x)}\geq {g(x)}$];
... latexmath:[$a^{f(x)}\leq a^{g(x)}\quad \Leftrightarrow\quad {f(x)}\leq {g(x)}$].
...
... latexmath:[$a^{f(x)}> a^{g(x)}\quad \Leftrightarrow\quad {f(x)}<{g(x)}$];
... latexmath:[$a^{f(x)}< a^{g(x)}\quad \Leftrightarrow\quad {f(x)}>{g(x)}$];
... latexmath:[$a^{f(x)}\geq  a^{g(x)}\quad \Leftrightarrow\quad {f(x)}\leq {g(x)}$];
... latexmath:[$a^{f(x)}\leq a^{g(x)}\quad \Leftrightarrow\quad {f(x)}\geq{g(x)}$].
...
\Leftrightarrow &\dfrac{5(5x+2)-4(x+1)}{20}\\\\
&\Leftrightarrow &\dfrac{21x+6}{20}>0 &\Leftrightarrow & \dfrac{3(7x+2)}{20}>0\\\\
&\Leftrightarrow & {7x+2}>0& \Leftrightarrow & x>-\dfrac{2}{7}.
\end{array}\]
++++
+
Portanto,
...
++++
\[\begin{array}{cccccc}
\left((0,3)^{(3x+2)(x+1)}\right)^\frac{1}{x+2}\geq \dfrac{(0,9)^{2x+5}}{3^{2x+5}}& \Leftrightarrow &
...
+
Lembre que, pela *nota* da *subseção 1.4.1.4*, latexmath:[$x+2\neq0$ implica que o intervalos que tenham $r_1$] devem ser abertos neste extremo. 
+
Portanto, 
+
[latexmath]
...
\]
++++
onde latexmath:[$a_0,\,\,a_1,\dots,a_n, b_0,\,\,b_1,\dots,b_m, c_0,\,\,c_1,\dots,c_l$] são contantes, latexmath:[$a_n> 0$, $b_m>0$ e $c_l>0$], latexmath:[$n$, $m$ e $l \in \mathbb{N}$]. Então, os casos particulares das inequações irracionais numa variável, que trabalharemos, são da forma:
...
+
--
... latexmath:[$\sqrt{P(x)}>Q(x)$] latexmath:[$ \quad \Leftrightarrow \quad$] latexmath:[$\Big( P(x)\geq 0\quad \mbox{e}\quad Q(x)\leq 0\Big)\quad \mbox{ou}\quad \Big(  P(x)\geq 0\quad \mbox{e}\quad P(x)>Q^2(x)\Big)$]; 
... latexmath:[$\sqrt{P(x)}\geq Q(x)$]latexmath:[$ \quad \Leftrightarrow \quad$]latexmath:[$\Big( P(x)\geq 0\quad \mbox{e}\quad Q(x)\leq 0\Big)\quad \mbox{ou}\quad \Big(  P(x)\geq 0\quad \mbox{e}\quad P(x)\geq Q^2(x)\Big)$];

... latexmath:[$\sqrt{P(x)}<Q(x)$]latexmath:[$ \quad \Leftrightarrow \quad$]latexmath:[$P(x)\geq 0\quad \mbox{e}\quad Q(x)> 0\quad \mbox{e}\quad  P(x)<Q^2(x)$]; 
... latexmath:[$\sqrt{P(x)}\leq Q(x)$]latexmath:[$ \quad \Leftrightarrow \quad$]latexmath:[$P(x)\geq 0\quad \mbox{e}\quad Q(x)\geq 0\quad \mbox{e}\quad P(x) \leq Q^2(x)$].
--
...
+
--
... latexmath:[$\sqrt{P(x)}+ \sqrt{Q(x)}>0$]latexmath:[$ \quad \Leftrightarrow \quad $]latexmath:[$\Big(P(x)\geq 0\,\,\,\mbox{e}\,\,\, Q(x)> 0\Big)\quad \mbox{ou}\quad \Big(P(x)> 0\,\,\,\mbox{e}\,\,\, Q(x)\geq 0\Big)$];
... latexmath:[$\sqrt{P(x)}+ \sqrt{Q(x)}\geq 0$]latexmath:[$ \quad \Leftrightarrow \quad$]latexmath:[$P(x)\geq 0\quad\mbox{e}\quad  Q(x)\geq 0$];

... latexmath:[$\sqrt{P(x)}\pm \sqrt{Q(x)}\geq k$], latexmath:[$k > 0$]latexmath:[$ \quad \Leftrightarrow \quad$]latexmath:[$P(x)\geq 0\quad\mbox{e}\quad  Q(x)\geq 0 \quad\mbox{e}\quad P(x)\geq (k\mp\sqrt{Q(x)})^2$];

... latexmath:[$\sqrt{P(x)}+ \sqrt{Q(x)}<0$]latexmath:[$ \quad \Leftrightarrow \quad$]latexmath:[${\rm C.\,\,S.}=\emptyset$];
... latexmath:[$\sqrt{P(x)}+ \sqrt{Q(x)}\leq 0$]latexmath:[$ \quad \Leftrightarrow \quad$]latexmath:[$P(x)= 0\quad\mbox{e}\quad Q(x)= 0$].
--
...
+
--
... latexmath:[$\sqrt{P(x)}- \sqrt{Q(x)}>0$]latexmath:[$ \quad \Leftrightarrow \quad$]latexmath:[$P(x)\geq 0\quad\mbox{e}\quad  Q(x)\geq  0\quad\mbox{e}\quad  P(x)>  Q(x)$];
... latexmath:[$\sqrt{P(x)}- \sqrt{Q(x)}\geq 0$]latexmath:[$ \quad \Leftrightarrow \quad$]latexmath:[$P(x)\geq 0\quad\mbox{e}\quad  Q(x)\geq 0\quad\mbox{e}\quad P(x)\geq   Q(x)$].
--
...
[latexmath]
++++
\[\begin{array}{ccllllllcccccc}
\sqrt{x^2-x-2}<5-x & \Leftrightarrow & {x^2-x-2}\geq 0 \quad \mbox{e}\quad 5-x \geq 0  \quad \mbox{e} \quad x^2-x-2<(5-x)^2
\\\\
...
\\\\
&\Leftrightarrow & (x-2)(x+1)\geq 0\quad \mbox{e}\quad 5 \geq x  \quad \mbox{e} \quad x<3.
\end{array}
\]
Logo,
\[\begin{array}{rcrcl}
(x-2)(x+1)&\geq& 0&\quad \Leftrightarrow \quad &  x \in (-\infty, -1]\cup[2,+\infty);\\
x &\leq& 5&\quad \Leftrightarrow \quad&   x \in (-\infty, 5];\\
x&<&3&\quad \Leftrightarrow \quad & x \in (-\infty,3).
\end{array}
\]
...
++++
\[\begin{array}{ccllll}
\sqrt{x+5}< 0& \Leftrightarrow & x+5 \geq 0 \quad \mbox{e} \quad 0> 0  \quad \mbox{e} \quad x+5< 0^2
\\\\
&\Leftrightarrow & x\geq -5 \quad \mbox{e} \quad 0> 0  \quad \mbox{e} \quad x+5<0.
\\\\
...
0&>& 0&\quad \Leftrightarrow \quad&   \emptyset;\\
x&<&-5&\quad \Leftrightarrow \quad & x \in (5,+\infty).
\end{array}
\]
Assim, $x$ pertence à interseção destes intervalos, isto é
\[
  x \in ( -\infty, -5]\cap\emptyset \cap( -5,+\infty ) =\emptyset
\]
Portanto,
\[
{\rm C.\,\,S.}=\emptyset
...
++++
+
Note que, não é necessário fazer as contas acima para obter que latexmath:[${\rm C.\,\,S.}=\emptyset$], pois da definição da raiz quadrada, se segue que latexmath:[$\sqrt{x+5}\geq 0$]. Assim,  latexmath:[$\sqrt{x+5}< 0$] é uma inequação não válida.
...
[NOTE]
Se a desigualdade a ser analisada tem a mesma forma que algum dos itens do *Caso IV*, porém ela é estrita, isto é,  latexmath:[$>$ ou $<$], então na  sua  inequação equivalente subtituímos latexmath:[$\geq $ ou $\leq $] por latexmath:[$>$ ou $<$], respectivamente.
...
...  latexmath:[$\sqrt[n\]{P(x)}\geq Q(x)$]latexmath:[$ \quad \,\Leftrightarrow  \quad \,$]latexmath:[$\Big(P(x)\geq 0 \quad \mbox{e} \quad Q(x)\leq 0 \Big)\quad \mbox{ou} \quad  \Big(P(x)\geq 0 \quad \mbox{e}\quad P(x)\geq Q^n(x)\Big)$];
...
[NOTE]
Se a desigualdade a ser analisada tem a mesma forma que algum dos itens do *Caso V*, porém ela é estrita, isto é,  latexmath:[$>$ ou $<$], então na  sua  inequação equivalente subtituímos latexmath:[$\geq $ ou $\leq $] por latexmath:[$>$ ou $<$], nas inequações que envolvam latexmath:[$P(x)$,  $Q(x)$ e $R(x)$], respectivamente.
...
\quad \Leftrightarrow\quad \dfrac{x+5}{(x-4)(x^2-81)}\geq 0
\]
Por outro lado, pelo item iii de {\bf Inequações Racionais} temos que 
\[
\begin{array}{cclc}
\dfrac{x+5}{(x-4)(x^2-81)}\geq 0&
\Leftrightarrow & (x+5){(x-4)(x^2-81)}\geq 0\quad \mbox{e}\quad (x-4)(x^2-81)\neq 0.
\end{array}
\]
No entanto,
\[
...
Assim,
\[
\dfrac{x+5}{(x-4)\sqrt[7]{81-x^2}}\leq 0\quad 
\Leftrightarrow \quad  (x+5){(x-4)(x-9)(x+9)}\geq 0\quad \mbox{e}\quad (x-4)(x-9)(x+9)\neq 0
\]
Fazendo $S(x)=(x+5)(x-4)(x-9)(x+9)$, temos que as raízes de $S(x)=0$ são $r_1=-9$, $r_2=-5$, $r_3=4$ e $r_4=9$. Pela {\bf nota} da {\bf subseção 1.4.1.4}, $(x-4)(x-9)(x+9)\neq 0$ implica que os intervalos que tenham $r_1$, $r_3$ e $r_4$ devem ser abertos nestes extremos. Desde que a inequação é da forma $S(x)\geq 0$,  podemos aplicar o item iii(a) do {\bf Caso I}, pois consideraremos todas as raízes.
...
Solução:: Desde que  latexmath:[$n=6$] é um número par, podemos aplicar o item iii do *Caso V*, para latexmath:[$P(x)=x+5$, $Q(x)=\sqrt[6\]{81-x^2}$ e $R(x)=x-4$], temos que:
+
[latexmath]
++++
\[
...
Assim, 
\[\begin{array}{ccl}
\dfrac{x+5}{(x-4)\sqrt[6]{81-x^2}}\geq 0
&\Leftrightarrow &   81-x^2> 0\quad\mbox{e}\quad (x+5)(x-4) \geq 0\quad \mbox{e}\quad x-4\neq 0\\\\
&\Leftrightarrow &   x^2-81< 0\quad\mbox{e}\quad (x+5)(x-4) \geq 0\quad \mbox{e}\quad x\neq 4\\\\
&\Leftrightarrow &  (x+9)(x-9)< 0\quad\mbox{e}\quad (x+5)(x-4) \geq 0\quad \mbox{e}\quad x\neq 4.
\end{array}\]
Logo,
\[\begin{array}{rcrcl}
(x+9)(x-9)&<& 0&\quad\Leftrightarrow\quad&  x \in (-9,9);\\
(x+5)(x-4) &\geq& 0&\quad\Leftrightarrow\quad&  x \in (-\infty,-5]\cap[4,+\infty);\\
x&\neq&4&\quad\Leftrightarrow\quad&  x \in (-\infty,4)\cup(4,+\infty).
\end{array}
\]
Assim, $x$ pertence à interseção destes intervalos, isto é
\[
  x \in ( -9,9)\cap\Big((-\infty,-5]\cap[4,+\infty\Big)\cap\Big(  (-\infty,4)\cup(4,+\infty) \Big) =(-9, -5]\cup (4,9).
\]
Portanto, 
\[{\rm C.\,\,S.}=(-9, -5]\cup (4,9).
\]
...
+
--
... Se latexmath:[$n_i> 0$ e par, para todo $i=1,\dots,k$], então temos as seguintes equivalências:
+
--
.. latexmath:[$\dfrac{P(x)}{R(x)\sqrt[n_1\]{Q_1(x)}\sqrt[n_2\]{Q_2(x)}\dots\sqrt[n_k\]{Q_k(x)}}\geq 0 \quad \,\Leftrightarrow $]
+
[latexmath]
++++
\[
...
.. latexmath:[$\dfrac{P(x)}{R(x)\sqrt[n_1\]{Q_1(x)}\sqrt[n_2\]{Q_2(x)}\dots\sqrt[n_k\]{Q_k(x)}}\leq 0 \quad \,\Leftrightarrow $]
+
[latexmath]
++++
\[
...
... Se latexmath:[$n_i> 1$ e impar, para todo $i=1,\dots,k$], então temos as seguintes equivalências:
+
--
.. latexmath:[$\dfrac{P(x)}{R(x)\sqrt[n_1\]{Q_1(x)}\sqrt[n_2\]{Q_2(x)}\dots\sqrt[n_k\]{Q_k(x)}}\geq 0$]latexmath:[$ \quad \,\Leftrightarrow  \quad \,$]latexmath:[$\dfrac{P(x)}{R(x)Q_1(x)Q_2(x)\dots Q_k(x)}\geq 0$];

.. latexmath:[$\dfrac{P(x)}{R(x)\sqrt[n_1\]{Q_1(x)}\sqrt[n_2\]{Q_2(x)}\dots\sqrt[n_k\]{Q_k(x)}}\leq 0$]latexmath:[$ \quad \,\Leftrightarrow  \quad \,$]latexmath:[$\dfrac{P(x)}{R(x)Q_1(x)Q_2(x)\dots Q_k(x)}\leq 0$].
--

... Se latexmath:[$n_i> 0$ e par, para todo $i=1,\dots,l$], e latexmath:[$n_i> 1$ e impar, para todo $i=l+1,\dots,k$]. Temos as seguintes equivalências:
+
--
.. latexmath:[$\dfrac{P(x)}{R(x)\sqrt[n_1\]{Q_1(x)}\sqrt[n_2\]{Q_2(x)}\dots\sqrt[n_l\]{Q_l(x)}\sqrt[n_{l+1}\]{Q_{l+1}(x)}\dots\sqrt[n_k\]{Q_k(x)}}\geq 0\quad \,\Leftrightarrow$]
+
[latexmath]
++++
\[
Q_1(x)>0\quad \mbox{e}  \quad\dots\quad \mbox{e} \quad Q_l(x)>0   \quad \mbox{e} \quad\dfrac{P(x)}{R(x)Q_{l+1}(x)\dots Q_k(x)}\geq 0;
\]
++++

.. latexmath:[$\dfrac{P(x)}{R(x)\sqrt[n_1\]{Q_1(x)}\sqrt[n_2\]{Q_2(x)}\dots\sqrt[n_l\]{Q_l(x)}\sqrt[n_{l+1}\]{Q_{l+1}(x)}\dots\sqrt[n_k\]{Q_k(x)}}\leq 0 \quad \,\Leftrightarrow $]
+
[latexmath]
++++
\[
Q_1(x)>0\quad \mbox{e}  \quad\dots\quad \mbox{e} \quad Q_l(x)>0  \quad \mbox{e} \quad\dfrac{P(x)}{R(x)Q_{l+1}(x)\dots Q_k(x)}\leq 0.
\]
++++
--
--
...
Solução:: Desde que  latexmath:[$n_1=4$, $n_2=2$ e $n_3=6$], ou seja todos são pares, podemos aplicar o item i(b) do *Caso VI*, para latexmath:[$P(x)=x^2-4$, $Q_1(x)=\sqrt[4\]{x^2-9}$, $Q_2(x)=\sqrt{x-1}$, $Q_3(x)=\sqrt[6\]{x-4}$ e $R(x)=x-13$]. Assim, temos que:
+
[latexmath]
++++
\[
...
\\\\\Leftrightarrow &
x^2-9> 0\quad\mbox{e}\quad x>1\quad\mbox{e}\quad x>4 \quad\mbox{e}\quad\dfrac{x^2-4}{x-13}\leq 0
\\\\\Leftrightarrow &
x^2-9> 0\quad\mbox{e}\quad x>4 \quad\mbox{e}\quad\dfrac{x^2-4}{x-13}\leq 0
\end{array}
\]
Por outro lado, pelo item iv de {\bf Inequações Racionais}, temos que
\[
...
\Leftrightarrow &
x^2-9> 0\quad \mbox{e}\quad x>4 \quad\mbox{e}\quad {(x^2-4)}{(x-13)} \leq 0\quad \mbox{e}\quad x\neq 13
\\\\\Leftrightarrow &  (x+3)(x-3)>0\quad\mbox{e}\quad x>4 \quad\mbox{e}\quad {(x-2)(x+2)}{(x-13)} \leq 0\quad \mbox{e}\quad x\neq 13.
\end{array}\]
Logo,
\[\begin{array}{rcrcl}
...
\[
 \begin{array}{c}
 \Big( (-\infty,-3)\cup(3,+\infty)\Big)\cap( 4,+\infty )\cap\Big( (-\infty,-2]\cup[2,13]\Big)\cap\Big((-\infty,13)\cup(13,+\infty)\Big)=(4,13).
\end{array}
\]
Portanto,
\[
...
Solução:: Desde que  latexmath:[$n_1=3$, $n_2=9$ e $n_3=5$], ou seja todos são impares, podemos aplicar o item ii(a) do *Caso VI*, para latexmath:[$P(x)=x^2-4$, $Q_1(x)=\sqrt[3\]{x^2-9}$, $Q_2(x)=\sqrt[9\]{x-1}$, $Q_3(x)=\sqrt[5\]{x-4}$ e $R(x)=x-13$]. Assim, temos que:
+
[latexmath]
++++
\[
\dfrac{x^2-4}{(x-13)\sqrt[3]{x^2-9}\sqrt[9]{x-1}\sqrt[5]{x-4}}\leq 0\quad \Leftrightarrow\quad  \dfrac{x^2-4}{(x-13)(x^2-9)(x-1)(x-4)}\leq 0\]
Por outro lado, pelo item iv de {\bf Inequações Racionais}, temos que
\[
\begin{array}{ccccc}
& \dfrac{x^2-4}{(x-13)(x^2-9)(x-1)(x-4)}\leq 0\\\\
\Leftrightarrow &(x^2-4)(x-13)(x^2-9)(x-1)(x-4)\leq 0\\\\
& \mbox{e}\quad(x-13)(x^2-9)(x-1)(x-4)\neq 0
\\\\\Leftrightarrow & (x-2)(x+2)(x-13)(x-3)(x+3)(x-1)(x-4)\leq 0\\\\
& \mbox{e}\quad(x-13)(x-3)(x+3)(x-1)(x-4)\neq 0.
\end{array}
\]
Fazendo $S(x)={(x-2)(x+2)}(x-13)(x-3)(x+3)(x-1)(x-4)$, temos que as raízes de $S(x)=0$ são $r_1=-3$, $r_2=-2$, $r_3=1$, $r_4=2$, $r_5=3$, $r_6=4$ e $r_7=13$. Pela {\bf nota} da {\bf subseção 1.4.1.4}, $(x-13)(x-3)(x+3)(x-1)(x-4)\neq 0$ implica que os intervalos que tenham $r_1$, $r_3$, $r_5$, $r_6$ e $r_7$ devem ser abertos nestes extremos. Desde que a inequação é da forma $S(x)\leq 0$, pelo item iv(b) do {\bf Caso I} de {\bf Inequações Polinomiais}, se segue que 
\[
...
\Leftrightarrow & x-1> 0\quad\mbox{e}\quad x-4>0 \quad\mbox{e}\quad\dfrac{x^2-4}{(x-13)(x^2-9)}\leq 0
\end{array}\]
Por outro lado, pelo item iv de {\bf Inequações Racionais}, temos que
\[
...
\[
 \begin{array}{cccccc}
&
x>1\quad\mbox{e}\quad x>4 \quad\mbox{e}\quad\dfrac{x^2-4}{(x-13)(x^2-9)}\leq 0
\\\\\Leftrightarrow &
x>1\quad\mbox{e}\quad x>4 \quad\mbox{e}\quad{(x^2-4)}{(x-13)(x^2-9)} \leq 0\quad \mbox{e}\quad (x-13)(x^2-9)\neq 0
\\\\\Leftrightarrow &
x>4 \quad\mbox{e}\quad {(x-2)(x+2)}{(x-13)(x+3)(x-3)} \leq 0\quad \mbox{e}\quad(x-13)(x+3)(x-3)\neq 0.
\end{array}
\]
Logo,
...
\[
\begin{array}{c}
(4,+\infty) \cap\Big((-\infty,-3]\cup[-2,2]\cup[3,13]\Big)\cap\Big((-\infty,-3)\cup(-3,3)\cup(3,13)\cup(13,+\infty)\Big)\\\\
=(4,13).
\end{array}
\]
Portanto,
...
++++
\[\begin{array}{ccccccc}
&|7-4x| =12& \Leftrightarrow & 7-4x =12\quad \mbox{ou}\quad 7-4x=-12\\\\
...
Denotemos por latexmath:[$E(x)$] a equação latexmath:[$|x-2|+3|x-4|=5|x+1|$]. Para determinar as raízes desta equação, igualaremos cada um destes valores absolutos a zero,  pois precisamos aplicar a definição do valor absoluto a cada termo. Fazendo isto, obtemos latexmath:[$x=2$], latexmath:[$x=4$] e 
...
* O *conjunto dos números aplicar aacionais*, denotado por  latexmath:[$\mathbb{I}$], é o conjunto 
+
[latexmath]
++++
\[
...
- Entre dois números irracionais existe um *conjunto enumerável* de números racionais.
====
