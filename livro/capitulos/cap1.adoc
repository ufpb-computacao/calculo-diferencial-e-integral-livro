...
+
--
.. latexmath:[$\dfrac{P(x)}{R(x)\sqrt[n_1\]{Q_1(x)}\sqrt[n_2\]{Q_2(x)}\dots\sqrt[n_k\]{Q_k(x)}}\geq 0$]latexmath:[$ \quad \,\equiv  \quad \,$]latexmath:[$Q_1(x)>0,\quad Q_2(x)>0,\quad\dots Q_k(x)>0,   \quad \mbox{e} \quad \dfrac{P(x)}{R(x)}\geq 0$];
...
.. latexmath:[$\dfrac{x+5}{(x-4)\sqrt[4\]{x^2-4}\sqrt{x-1}\sqrt[6\]{x+3}}\leq 0$]

Solução:: Desde que  latexmath:[$n_1=4$, $n_2=2$ e $n_3=6$], ou seja todos pares, podemos aplicar o item i(b) do *Caso VI*, para latexmath:[$P(x)=x+5$, $Q_1(x)=\sqrt[4\]{x^2-4}$, $Q_2(x)=\sqrt{x-1}$, $Q_3(x)=\sqrt[6\]{x+3}$ e $R(x)=x-4$]. Assim, temos que:
+
[latexmath]
++++
\[
