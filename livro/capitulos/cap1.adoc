...
++++
+ 
Portanto, aplicando o  item(iii) do *Caso II*, pois latexmath:[$r_1<r_2$], latexmath:[$C.\,\,S.= \big(-\infty,-\frac{3}{4}\big\]\cup \big[1,+\infty\big)$].
...
--
+
... latexmath:[$P(x)<0$] se, e somente se, latexmath:[$x$] pertence à união dos intervalos aberots com sinal latexmath:[$-$], isto é:
+
--
...
++++
+
Logo, pelo *Caso III*, latexmath:[$x^2+16$ e $x^4+1$] não serão consideradas na obtenção dos intervalos que definem  latexmath:[$C.\,\,S.$]
+
Assim, latexmath:[$(x^2-3)^5(x^2+16)(x^2-16)(x^4+1)> 0$ $\,\,\,\Leftrightarrow\,\,\,$ $(x+\sqrt{3})^5(x-\sqrt{3})^5(x-4)(x+4)> 0$]. Logo, resolvendo a equação latexmath:[$(x+\sqrt{3})^5(x-\sqrt{3})^5(x-4)(x+4)= 0$], temos que latexmath:[$r_1=-4$], latexmath:[$r_2=-\sqrt{3}$],  latexmath:[$r_3=\sqrt{3}$] e  latexmath:[$r_4=4$]. Note que tanto latexmath:[$r_2$ como $r_3$] têm multiciplicidade latexmath:[$5$]. Do *Caso II* item(ii) latexmath:[$r_2$ e $r_3$] serão consideradas para a obtenção dos intervalos que definem o latexmath:[$C.\,\,S.$]. Mais ainda, como a inequação é da forma latexmath:[$P(x)> 0$], podemos aplicar o item(ia) do *Caso I*, para todas as raízes  latexmath:[$r_1=-4$], latexmath:[$r_2=-\sqrt{3}$],  latexmath:[$r_3=\sqrt{3}$] e latexmath:[$r_4=4$]. Ou seja, então latexmath:[$x$] pertence à união dos intervalos com sinal latexmath:[$(+)$]. Veja figura a seguir:
+
...
\]
++++ 
Para resolver este tipo de inequações, devemos ter em mente que:
...
Logo, para obter o conjunto solução procedemos como nos casos anteriores. 
...
++++ 
+
Assim, as raízes da última inequação equivalente são latexmath:[$r_1=0$] e latexmath:[$r_2=4$]. Como a inequação é da forma latexmath:[$P(x)> 0$],  podemos aplicar o item i(a) do *caso I*, pois  consideraremos todas as raízes. Ou seja, latexmath:[$x$] pertence à união dos intervalos com sinal latexmath:[$(+)$], conforme a figura a seguir:
+
...
\[
\dfrac{(2x^2-x+1)(x+2)}{(x-1)x(x+1)}\leq 0\quad \Leftrightarrow\quad {(2x^2-x+1)(x+2)}{(x-1)x(x+1)}\leq 0\quad \mbox{e}\quad (x-1)x(x+1)\neq 0
\]
++++ 
+
Como latexmath:[$2x^2-x+1=0$  não tem raízes reais, esta expressão não será considerada para a obtenção de $C.\,S.$]. Assim, as raízes dos fatores são latexmath:[$r_1=-2$], latexmath:[$r_2=-1$], latexmath:[$r_3=0$] e 
latexmath:[$r_4=1$]. Note que latexmath:[$(x-1)x(x+1)\neq 0$ implica que $x\neq-1, 0,1$]. Desde que a inequação é da forma latexmath:[$P(x)\leq 0$],  podemos aplicar o item iii(a) do *caso I*, pois  consideraremos todas as raízes. Ou seja, latexmath:[$x$] pertence à união dos intervalos com sinal latexmath:[$(-)$], veja figura a seguir:
+
